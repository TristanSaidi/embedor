...
def umap_tsne_param_abl(n_points):
    print(f"Running UMAP parameter ablation with {n_points} points.")
    save_path = '/burg/iicd/users/tls2160/research/Fa24/isorc/outputs/developmental_abl'
    ...
    exp_dict = {"min_dist":{}, "nsr":{}, "perplexity": {}, "ee":{}, "embedor":{}, "data": developmental_data, "labels": days}
    exp_dict["embedor"]["embedding"] = embedding

    # compute spearman and pearson correlation for the EmbedOR embedding
    spearman_corr, pearson_corr = compute_correlation(embedding, days)
    ...
    for min_dist in min_dists:
        ...
        umap_emb = umap.UMAP(n_neighbors=15, min_dist=min_dist, metric='euclidean').fit_transform(developmental_data)
        umap_emb = umap_emb[mask]  # apply the mask to the embedding
        spearman_corr, pearson_corr = compute_correlation(umap_emb, days)
    ...
    for nsr in negative_sample_rate:
        ...
        umap_emb = umap.UMAP(n_neighbors=15, min_dist=0.1, metric='euclidean', negative_sample_rate=nsr).fit_transform(developmental_data)
    ...
    for p in perplexity:
        ...
        tsne_emb = TSNE(n_components=2, perplexity=p, random_state=0).fit_transform(developmental_data)
        tsne_emb = tsne_emb[mask]
        spearman_corr, pearson_corr = compute_correlation(tsne_emb, days)
    ...
    for ee in early_exaggeration:
        ...
        tsne_emb = TSNE(n_components=2, perplexity=30, early_exaggeration=ee, random_state=42).fit_transform(developmental_data)
        tsne_emb = tsne_emb[mask]
        spearman_corr, pearson_corr = compute_correlation(tsne_emb, days)
    ...
    # save the exp_dict
    with open(os.path.join(developmental_path, 'umap_param_ablation.pkl'), 'wb') as f:
...
if __name__ == "__main__":
    ...
    seed = args.seed    
    np.random.seed(seed)
    umap_tsne_param_abl(args.n_points)