...
def benchmark_datasets(n_points):
    ...
    embedor_euc = EmbedOR(exp_params, fast=False, metric='euclidean')
    ...
    embedor_euc_path = os.path.join(kmnist_path, 'embedor_euc')
    os.makedirs(embedor_euc_path, exist_ok=False)
    plt.figure(figsize=(10, 10))
    plot_graph_2D(embedding_euc, embedor_euc.G, node_color=kmnist_labels[embedor_euc.G.nodes()], edge_width=0, node_size=0.1, edge_color='red')
    plt.savefig(os.path.join(embedor_euc_path, 'class_annot.png'))
    plt.close()
    plt.figure(figsize=(10, 10))
    plot_graph_2D(embedding_euc, low_energy_graph, node_color=None, edge_width=0.1, node_size=0.1, edge_color='green')
