...
def linRepulsion(n1, n2, coefficient=0):
    ...
    n1_pos = np.asarray(n1.position)
    n2_pos = np.asarray(n2.position)
...
def linRepulsion_region(n, r, coefficient=0):
    ...
    position = np.asarray(n.position)
    massCenter = np.asarray(r.massCenter)
...
def linAttraction(n1, n2, e, distributedAttraction, coefficient=0):
    ...
    position1 = np.asarray(n1.position)
    position2 = np.asarray(n2.position)
...
class Region:
    ...
    def updateMassAndGeometry(self):
        if len(self.nodes) > 1:
            ...
            # massSumX = 0
            # massSumY = 0
            massSum = np.zeros(np.asarray(self.massCenter).shape)
            for n in self.nodes:
                self.mass += n.mass
                # massSumX += n.x * n.mass
                # massSumY += n.y * n.mass
            ...
            for n in self.nodes:
                ...
                position = np.asarray(n.position)
                massCenter = np.asarray(self.massCenter)
...
def adjustSpeedAndApplyForces(nodes, speed, speedEfficiency, jitterTolerance):
    ...
    for n in nodes:
        ...
        old_delta = np.asarray(n.old_delta)
        delta = np.asarray(n.delta)
